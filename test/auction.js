...
contract("Auction", function(accounts) {
    ...
    it("allows a bidder to cast a bid", function() {
        return Auction.deployed().then(function(instance) {
            auctionInstance = instance;
          itemId = 1;
          return auctionInstance.bid(itemId,3000 , { from: accounts[0] });
          ...
          var bidValue = item[2];
          assert.equal(bidValue,3000, "updates the bidder's bid");
        })
    });
    ...
    it("throws an exception for invalid items", function() {
        return Auction.deployed().then(function(instance) {
            auctionInstance = instance;
          return auctionInstance.bid(99,3000, { from: accounts[1] })
        }).then(assert.fail).catch(function(error) {
          assert(error.message.indexOf('revert') >= 0, "error message must contain revert");
          return auctionInstance.items(1);
        }).then(function(item1) {
          var bidValue = item1[2];
          assert.equal(bidValue, 3000, "item 1 did not receive any bid value");
          return auctionInstance.items(2);
        }).then(function(item2) {
    ...
    it("throws an exception for double bid", function() {
        return Auction.deployed().then(function(instance) {
          auctionInstance = instance;
          itemId = 2;
          auctionInstance.bid(itemId, 100000, { from: accounts[1] });
          return auctionInstance.items(itemId);
        }).then(function(item) {
          var bidValue = item[2];
          assert.equal(bidValue, 100000, "accepts first bidding");
          // Try to vote again
          return auctionInstance.bid(itemId, 150000, { from: accounts[1] });
        }).then(assert.fail).catch(function(error) {
          assert(error.message.indexOf('revert') >= 0, "error message must contain revert");
          return auctionInstance.items(1);
        }).then(function(item1) {
          ...
          var bidValue = item2[2];
          assert.equal(bidValue, 100000, "item 2 did not receive any bid value");
        });
    });
});